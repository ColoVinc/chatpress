...
/**
 * Vcai_History — gestisce conversazioni e messaggi nel DB
 */
class Vcai_History {
    ...
    public static function create_conversation( int $user_id, string $first_message ): int {
        ...
        $wpdb->insert(
            $wpdb->prefix . 'vcai_conversations',
    ...
    public static function get_conversations( int $user_id, int $limit = 50 ): array {
        ...
        return $wpdb->get_results( $wpdb->prepare(
            ...
            "SELECT c.*, COUNT(m.id) as message_count
             FROM {$wpdb->prefix}vcai_conversations c
             LEFT JOIN {$wpdb->prefix}vcai_messages m ON m.conversation_id = c.id
             WHERE c.user_id = %d
             GROUP BY c.id
    ...
    public static function get_messages( int $conversation_id, int $user_id ): array {
        ...
        $owner = $wpdb->get_var( $wpdb->prepare( "SELECT user_id FROM {$wpdb->prefix}vcai_conversations WHERE id = %d", $conversation_id ) );
        if ( (int) $owner !== $user_id ) return [];

        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- custom table
        return $wpdb->get_results( $wpdb->prepare(
            "SELECT role, content, created_at FROM {$wpdb->prefix}vcai_messages WHERE conversation_id = %d ORDER BY created_at ASC",
            $conversation_id
        ), ARRAY_A );
    }
    ...
    public static function delete_conversation( int $conversation_id, int $user_id ): bool {
        ...
        $wpdb->delete( $wpdb->prefix . 'vcai_messages', [ 'conversation_id' => $conversation_id ], [ '%d' ] );
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- custom table, delete operation
        $wpdb->delete( $wpdb->prefix . 'vcai_conversations', [ 'id' => $conversation_id ], [ '%d' ] );
        return true;
    }
    ...
    public static function delete_older_than( int $days ): int {
        ...
        $ids = $wpdb->get_col( $wpdb->prepare(
            "SELECT id FROM {$wpdb->prefix}vcai_conversations WHERE updated_at < DATE_SUB(NOW(), INTERVAL %d DAY)",
            $days
        ) );
        ...
        foreach ( $ids as $id ) {
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- custom table, delete operation
            $wpdb->delete( $wpdb->prefix . 'vcai_messages', [ 'conversation_id' => $id ], [ '%d' ] );
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- custom table, delete operation
            $wpdb->delete( $wpdb->prefix . 'vcai_conversations', [ 'id' => $id ], [ '%d' ] );
        }
